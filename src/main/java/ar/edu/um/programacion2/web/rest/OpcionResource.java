...
public class OpcionResource {
    ...
    private final Logger log = LoggerFactory.getLogger(OpcionResource.class);
    ...
    public ResponseEntity<OpcionDTO> createOpcion(@Valid @RequestBody OpcionDTO opcionDTO) throws URISyntaxException {
        ...
        return ResponseEntity
            .created(new URI("/api/opcions/" + result.getId()))
     ...
     * @throws URISyntaxException if the Location URI syntax is incorrect.
     */
    @PatchMapping(value = "/opcions/{id}", consumes = { "application/json", "application/merge-patch+json" })
     ...
     */
    @GetMapping("/opcions")
    public ResponseEntity<List<OpcionDTO>> getAllOpcions(@org.springdoc.api.annotations.ParameterObject Pageable pageable) {
    ...
    public ResponseEntity<OpcionDTO> getOpcion(@PathVariable Long id) {
